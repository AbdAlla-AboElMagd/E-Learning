  ...
  const PostEnrolled = async (username, id) => {
    ...
    try {
      const response = await axios.post(
        "https://api-generator.retool.com/QpLwM6/data",
        data,
        {
          headers: {
  ...
  const checkEnrolled = async (username, id) => {
    if (isLoggedIn) {
      try {
        ...
        const EnrolledCourses = response.data.some(
    ...
    <Container>
      <Box mt={4}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={4}>
            <Paper elevation={12} sx={{ p: 3 }}>
              ...
              {message && (
                <Box mt={2}>
                  <Alert severity={isEnrolled ? "success" : "info"}>
                    {message}
                  </Alert>
                </Box>
              )}
        ...
        <Paper elevation={12} sx={{ p: 3, mt: 3 }}>
          ...
          {Array.from(
            { length: Math.floor(Math.random() * (10 - 3 + 1)) + 3 },
            (_, index) => (
